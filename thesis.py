...
POWERCONJUCTIONS = ['therefore', 'while', 'although', 'because', 'nevertheless', 'wherein']
BOUNDS = [',', '"', "––", " - ", ":", ";"]
...
def thesisVector(thesis, essentialNouns, essentialVerbs):
	...
	pNoun = len(essentialNouns)*1.0/len(nouns)
	pVerb = len(essentialVerbs)*1.0/len(nouns)
	pPronoun = len(pronouns)*1.0/len(nouns)
	return { "EssentialNouns" : pNoun, "EssentialVerbs" : pVerb, "Bounded" : pBound(words), "Pronoun":pPronoun}
